    ...
    dbc.Row([
        ...
        dbc.Col([
            html.Div([
                ...
                dcc.Slider(id='WindowSize-slider1',
                            min=1,  
                            max=50,
                            step=1,
                            value=15),
                html.Div(id='WindowSize-slider-output-container1')        
                    ]),
        ...
        dbc.Col([
            html.Div([
                html.H3("Step Size"),
                dcc.Slider(id='StepSize-slider1',
                            min=0,
                            max=50,
                            step=1,
                            value=10),
                html.Div(id='StepSize-slider-output-container1')        
                    ]),
        ],# width={'size':3, 'offset':1, 'order':1},
           xs=12, sm=12, md=12, lg=5, xl=5
        ),
...
@app.callback(
    dash.dependencies.Output('WindowSize-slider-output-container1', 'children'),
    [dash.dependencies.Input('WindowSize-slider1', 'value')])
def update_output(value):
    return 'You have selected {}'.format(value)

@app.callback(
    dash.dependencies.Output('StepSize-slider-output-container1', 'children'),
    [dash.dependencies.Input('StepSize-slider1', 'value')])
def update_output(value):
    return 'You have selected {}'.format(value)
    ...
    try:
        if 'fasta' in filename:
            ...
            return html.Div([
                        ...
                        html.Button(id="submit-button1", children="Create Graph"),
                        html.Hr(),
...
@app.callback(Output('output-fasta1', 'children'),
